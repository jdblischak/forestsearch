...
generate_aft_dgm_flex_enhanced <- function(data,
                                 continuous_vars,
                                 factor_vars,
                                 outcome_var,
                                 event_var,
    ...
    result <- list(
      ...
      model_params = list(
        ...
        censoring = list(
          type = "weibull",
          mu = muC,
          sigma = tauC  # Changed from tau to sigma for consistency
        )
      ),
      spline_info = list(
        knot = knot,
  ...
  # Build model data frame with all predictors
  model_data <- df_analysis

  # Add treatment variable if needed
  if (!treatment_var_use %in% names(model_data)) {
    model_data[[treatment_var_use]] <- df_analysis[[treatment_var_use]]
  }
  ...
  for (i in seq_along(continuous_vars)) {
    ...
    if (standardize) {
      model_data[[paste0(var_name, "_std")]] <- scale(df_analysis[[var_name]])[,1]
    } else {
      model_data[[paste0(var_name, "_std")]] <- df_analysis[[var_name]]
    }
  }

  # Add factor dummies if they exist
  if (!is.null(factor_dummies)) {
    for (j in seq_len(ncol(factor_dummies))) {
      model_data[[colnames(factor_dummies)[j]]] <- factor_dummies[,j]
    }
  }

  # Build formula string
  predictor_names <- c(treatment_var_use)

  # Add continuous variables (standardized versions)
  predictor_names <- c(predictor_names, paste0(continuous_vars, "_std"))

  # Add factor dummies
  if (!is.null(factor_dummies)) {
    predictor_names <- c(predictor_names, colnames(factor_dummies))
  }

  # Add interaction term if alt model
  if (model == "alt") {
    model_data$treat_x_subgroup <- df_analysis$treat_x_subgroup
    predictor_names <- c(predictor_names, "treat_x_subgroup")
  }

  # Create formula
  formula_str <- paste0("Surv(", outcome_var, ", ", event_var, ") ~ ",
                       paste(predictor_names, collapse = " + "))
  aft_formula <- as.formula(formula_str)

  # Fit model
  aft_fit <- survreg(aft_formula, data = model_data, dist = "weibull")
  ...
  # Create a mapping of coefficient names
  # The coefficients will have names like "treat", "age_std", etc.
  coef_names <- names(gamma_aft)

  # Apply k_treat and k_inter modifications
  treat_idx <- which(coef_names == treatment_var_use)
  if (length(treat_idx) > 0) {
    gamma_aft[treat_idx] <- gamma_aft[treat_idx] * k_treat
    if (verbose && k_treat != 1) {
      cat("Treatment effect modified by factor:", k_treat, "\n")
    }
  }

  interact_idx <- which(coef_names == "treat_x_subgroup")
  if (length(interact_idx) > 0 && model == "alt") {
    gamma_aft[interact_idx] <- gamma_aft[interact_idx] * k_inter
  ...
  treat_idx <- which(names(gamma_aft) == treatment_var_use)
  if (length(treat_idx) > 0) {
    hr_overall <- exp(b_weibull[treat_idx])
  } else {
    hr_overall <- 1
    warning("Treatment effect not found in model")
  }

  if (model == "alt" && !is.null(subgroup_vars)) {
    # HR in harm subgroup (treatment + interaction)
    interact_idx <- which(names(gamma_aft) == "treat_x_subgroup")
    if (length(interact_idx) > 0) {
      hr_harm_subgroup <- exp(b_weibull[treat_idx] + b_weibull[interact_idx])
    } else {
      hr_harm_subgroup <- hr_overall
    }

    # HR in no-harm subgroup (treatment only)
    hr_no_harm_subgroup <- hr_overall
  ...
  # Prepare covariate matrix for super population
  # We need to match the structure used in model fitting

  # Standardize continuous variables if needed
  X_super_std <- df_super[, continuous_vars, drop = FALSE]
  if (standardize) {
    for (var in continuous_vars) {
      X_super_std[[var]] <- scale(X_super_std[[var]])[,1]
    }
  }
  ...
  if (length(factor_vars) > 0 && !is.null(factor_dummies)) {
    ...
    # Store dummies in super population
    for (j in seq_len(ncol(factor_dummies_super))) {
      df_super[[colnames(factor_dummies_super)[j]]] <- factor_dummies_super[,j]
    }
  }

  # Calculate linear predictors
  # Start with intercept
  df_super$lp_base <- mu_aft

  # Add continuous variable contributions
  for (var in continuous_vars) {
    coef_name <- paste0(var, "_std")
    if (coef_name %in% names(gamma_aft)) {
      df_super$lp_base <- df_super$lp_base + X_super_std[[var]] * gamma_aft[coef_name]
    }
  }

  # Add factor dummy contributions
  if (!is.null(factor_dummies_super)) {
    for (dummy_name in colnames(factor_dummies_super)) {
      if (dummy_name %in% names(gamma_aft)) {
        df_super$lp_base <- df_super$lp_base +
                           df_super[[dummy_name]] * gamma_aft[dummy_name]
      }
    }
  }

  # Add treatment effects
  treat_coef <- gamma_aft[treatment_var_use]
  if (is.na(treat_coef)) treat_coef <- 0

  if (model == "alt" && !is.null(subgroup_vars)) {
    interact_coef <- gamma_aft["treat_x_subgroup"]
    if (is.na(interact_coef)) interact_coef <- 0

    df_super$lp_treat_1_in <- df_super$lp_base + treat_coef +
                              df_super$in_subgroup * interact_coef
    df_super$lp_treat_1_out <- df_super$lp_base + treat_coef
    df_super$lp_treat_0 <- df_super$lp_base
  } else {
    df_super$lp_treat_1 <- df_super$lp_base + treat_coef
    df_super$lp_treat_0 <- df_super$lp_base
  }
