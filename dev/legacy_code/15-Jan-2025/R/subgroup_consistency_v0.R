...
if(sg_focus=="hr") setorder(result_new,-Pcons,-hr,K)
# Choose sg with largest size 
# Emphasis by hrMaxSG, consistency, and smallest K
if(sg_focus %in% c("hrMaxSG","maxSG")) setorder(result_new,-N,-Pcons,K)
if(sg_focus %in% c("hrMinSG","minSG")) setorder(result_new,N,-Pcons,K)
...
# For df.est --> only return "id" and "treat.recommend"  
out<-list(result=copy(result_new),sg.harm=this.1,sg.harm_label=this.1_label,df_flag=data.found[,c("id","treat.recommend")],sg.harm.id=grp1)
return(out)
}

# sg_focus = hr --> sort by HR's to prioritize largest HR's
# sg_focus= hrMaxSG ---> prioritize SG size
...
# Note: is set to < 0 for sg_focus=hrMaxSG or hrMinSG in forestsearch()

# Remove sg_focus, will return all options
# Add Lsg = number of possible factors ("L" in paper)
# Add stop_Kgroups to stop when number of identified subgroups meeting consistency exceeds stop_Kgroups
# That is, Stopping for futility OR reaching stop_Kgroups (meeting Kgroups is combined with futility flag)
...
stop.threshold=1.1,by.risk=2,plot.sg=FALSE,maxk=7,pstop_futile=-0.1,Lsg,confs_labels,sg_focus="hr",stop_Kgroups=10){
...
if(nrow(found.hrs)>1){
# Remove redundancies: sort by HRs and remove ties
found.new <- found.hrs[order(found.hrs$HR,decreasing=TRUE),]
# Keep first
f1.hrs <- found.new[1,]
temp <- found.new[-c(1),]
temp2<- as.matrix(found.new[,c("HR","n","E","K","L(HR)","U(HR)")])
# If equal across these components then consider redundant
id_keep <- which(round(diff(temp2,lag=1),6)!=0)
fkeep.hrs <- temp[id_keep,] 
found.hrs <- na.omit(data.table(rbind(f1.hrs,fkeep.hrs)))
}

# sort by HRs unless maxSG or minSG
# This is done above
#if(!(sg_focus %in% c("maxSG","minSG"))) found.hrs<-found.hrs[order(found.hrs$HR,decreasing=TRUE),]
...
stop_futile <- FALSE
  ...
  # Since HR's are sorted, chance of meeting 90% likely "futile"
    if(p.consistency < stop.threshold & !stop_futile){
      ...
      # Trigger stopping for futility OR reaching stop_Kgroups
      if(p.consistency <= pstop_futile | (any.found==(stop_Kgroups-1))){
       stop_futile <- TRUE
      }
  
      if(p.consistency < pconsistency.threshold & details) cat("Kmet (found), Subgroup, % Consistency=",c(any.found,this.m_label,p.consistency),"\n")
      ...
      if(details){
        cat("# of splits, Kmet=",c(n.splits,any.found),"\n")
...
out_maxSG <- sg_consistency_out(df=df,result_new=result_new,sg_focus="maxSG",details=sgdetails,plot.sg=sgdetails,index.Z=index.Z,names.Z=names.Z,by.risk=by.risk,confs_labels=confs_labels)

# sg_focus="hrMinSG"
sgdetails<-ifelse(plot.sg & sg_focus %in% c("hrMinSG","minSG"), TRUE, FALSE)
out_minSG <- sg_consistency_out(df=df,result_new=result_new,sg_focus="minSG",details=sgdetails,plot.sg=sgdetails,index.Z=index.Z,names.Z=names.Z,by.risk=by.risk,confs_labels=confs_labels)
...
if(sg_focus %in% c("hrMaxSG","maxSG")){
  df_flag <- out_maxSG$df_flag
  this.1 <- out_maxSG$sg.harm_label
  sg.harm.id <- out_maxSG$sg.harm.id
}
if(sg_focus %in% c("hrMinSG","minSG")){
  df_flag <- out_minSG$df_flag
  this.1 <- out_minSG$sg.harm_label
  sg.harm.id <- out_minSG$sg.harm.id
}
...
out<-list(out_hr=out_hr,out_maxSG=out_maxSG,out_minSG=out_minSG,df_flag=df_flag,sg.harm=this.1,sg.harm.id=sg.harm.id)
return(out)
}
...
plot.subgroup<-function(tte.name,event.name,treat.name,weight.name=NULL,
                        ...
                        exp.lab="Experimental",con.lab="Control",legend.cex=0.60,risk.cex=0.60,
