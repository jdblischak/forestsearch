...
cox_cs_fit <- function(df,tte.name=c("os_time"),event.name=c("os_event"),treat.name=c("treat"),strata.name=c("stratum"),z.name=c("bm"),alpha=0.20,details=FALSE,boots=0,
xlab=c("z"), maxz = Inf, minq=0.0, maxq=1.0, add.points=NULL, 
show_plot=TRUE, truebeta.name=NULL,ypadzero=0.01,ydel=0.175,cex_legend=1.0,zwindow=0.0,byz=1,ylimit=NULL,main_title=NULL){
...
z_q <- c(quantile(z,c(minq,0.75,0.80,0.90,maxq),na.rm=TRUE))

if(details) cat("Z quantiles at 75%, 80%, 90%:",c(z_q[2:4]),"\n")

# log(HR) estimates as function of Z estimated at:

z_profile <- seq(z_q[1],z_q[5],by=byz)
# cutoff at maxz
z_profile <- unique(pmin(z_profile,rep(maxz,length(z_profile))))
z_profile <- sort(unique(c(add.points,z_profile)))
# Standard Cox
...
legend("top", c("log(HR)","80% CI"),lty=c(2,2), lwd=c(2,1), col=c("black","black"), bty='n',cex=cex_legend)
...
text(c(z_profile),c(yycounts),c(counts_profile),col="black",cex=0.7)
}
