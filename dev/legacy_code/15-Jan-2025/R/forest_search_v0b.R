...
#if(any(grep("_",Allconfounders.name)) | any(grep("-",Allconfounders.name))) stop("confounders names with underscores or hyphenation not allowed (please change [eg] var_1 to var1")  
...
var_names<-c(Allconfounders.name,outcome.name,event.name,id.name,treat.name)
