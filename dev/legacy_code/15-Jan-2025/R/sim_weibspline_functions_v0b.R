...
get_dgm_stratified <- function(df,knot=5,kappa=10,hrs=c(0.75,0.75,0.75),details=TRUE,strata_tte=NULL,confounders=NULL,masked=TRUE){
  ...
  # To simplify we will take median ("sig_approx" below)
  
  mu <- c(coef(fit.weibk)[1])
  gamma <- c(coef(fit.weibk)[c(-1)])
  sig <- c(fit.weibk$scale) # We rename tau (in notes) to sig 
  ...
  if(!is.null(strata_tte)){
    ...
    sig.strataO <- sig[sig_ids]
    if(length(sig.strataO)!=nrow(dfa2)) stop("strata_tte not uniquely identified via matching")
    sig.approx <- median(sig.strataO)
    dfa2$sig.strataO <- sig.strataO
  }
  ...
  a0 <- c(-gamma)/sig.approx
  a0[1] <- loghr.0
  a0[3] <- (loghr.knot-loghr.0)/knot
  a0[5] <- (loghr.kappa-a0[1]-kappa*a0[3])/knot
  # back-solve for psi on AFT (log(T)) parameterization 

  psi.true <- -a0*sig.approx 

  # predictions log(hr)
  dfp <- dfa2[order(dfa2$z,decreasing=FALSE),]
  
  return(list(df_super=dfp,psi.true=psi.true,mu=mu,sig=sig,muC=muC,sigC=sigC,strata_tte=strata_tte,sig.approx=sig.approx))
}
...
draw_sim_stratified <- function(dgm,ss=1,details=FALSE,Ndraw=nrow(dgm$df_super),strata_rand=c("strataNew"),xname=c("ecogbl"),bx=0,checking=FALSE){
  ...
  # Outcomes
  # Regression parameters on AFT scale (psi)
  psi.true <- dgm$psi.true
  mu <- dgm$mu
  sig <- dgm$sig
  # Censoring
  muC <- dgm$muC 
  ...
  # These are outcome sigs for each subject
  sig.strataO <- df_super$sig.strataO
  ...
  eta1 <- mu + c(zmat.1%*%psi.true)+x*bx
  # Weibull log(hazard ratio) setting treat=1
  # and excluding mu (since taking difference below)
  phi1 <- -c(zmat.1%*%psi.true+x*bx)/sig.strataO
  log.Y1 <- eta1 + sig.strataO*epsilon
  
  # Setting treat=0
  z.0 <- zmat.0[,"z"]
  eta0 <- mu + c(zmat.0%*%psi.true)+x*bx
  log.Y0 <- eta0 + sig.strataO*epsilon
  phi0 <- -c(zmat.0%*%psi.true+x*bx)/sig.strataO
...
cox_cs_fit2 <- function(df,tte.name=c("os_time"),event.name=c("os_event"),treat.name=c("treat"),strata.name=c("stratum"),z.name=c("bm"),alpha=0.20,details=FALSE,boots=0,
                        xlab=c("z"),maxz = Inf, show_plot=TRUE,truebeta.name=NULL,ypadzero=0.01,ydel=0.25,cex_legend=1.0,zwindow=0.0,byz=1,ylimit=NULL,main_title=NULL,cex_count=0.7){
