...
sg_consistency_out<-function(df,result_new,sg_focus,index.Z,names.Z,details=FALSE,plot.sg=FALSE,by.risk=4,confs_labels){
  ...
  if(details & plot.sg){
    plot.subgroup(sub1=df.sub,sub1C=df.subC,tte.name="Y",event.name="Event",treat.name="Treat",subid=paste(this.1_label,collapse=" & "),byrisk=by.risk, put.legend.arms='top')
    print(this.1_label)
