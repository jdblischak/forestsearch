...
confounders.name <- c("age","bm","male","ecog","region_other","region_asia","histology","prior_treat","strat")
...
dgm_spline <- generate_aft_dgm_flex(
  ...
  factor_vars = c("male", "histology", "prior_treat","strat","AP","region_other"),
  ...
  spline_spec = list(
    ...
    knot = 1,
    zeta = 5,
    log_hrs = log(c(2.0, 0.9, 0.6))  # HR varies with ER
  ),
...
cat("AP region parmeter = -log(4)?", c(dgm_spline$model_params$b0["z_AP"],-log(4)),"\n")
...
plot_weighted_km(dfc1, conf.int = TRUE, show.logrank = TRUE, ymax = 1.05)
...
and challenges with establishing consistency within the AP region are induced by a strong prognostic effect ($\beta_{w}=-\log(4)$).
