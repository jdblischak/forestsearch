...
Keynote 590: Subjects with advanced/metastatic esophageal cancer


Exploratory subgroup analyses for the *squamous cell carcinoma* population based on overall survival outcomes will be conducted.

*   The following baseline factors were considered:
1. `Region: Asia vs non-Asia`
2. `Age (continuous)`
3. `Age <65yrs / >=65Yrs`
4. ` Male/Female`
5. `Ecog 0/1`
...
7. `Metastatic`
8. `Tumor size`
9. `Chemo (5-FU + cisplatin vs CAPOX)`
10. `CPS>=1, CPS>=5, CPS>=10`
11. `CPS>1, CPS>5, CPS>10, CPS>15`
12. `CPS (continuous)`

## Executive Summary of Identified Subgroups (non-stratified Cox)

Post-hoc subgroup analyses identified subgroups based on CPS cuts alone and in combination with tumor size (">=1", ">1", ">3", "tumor_size>19", "tumor_size>62").  Cross-validation 
assessments of *reproducibility* suggests the algorithm corresponding to the identification of the subgroup **CPS>1 and tumor_size>19** as **benefiting** appears to have (relatively) favorable properties.  Though we note 
that the cross-validation (CV) summary of reproducibility is in the $75\%$ range which may be borderline performance (e.g., ~ $76\%$ of the **CPS>1 and tumor_size>19** subjects classified as **benefiting** were also classified as benefiting in the CV testing sample [See `OS Subgroups Identified' section below]). 

- Overall Survival--
  - Overall (SCC, n=548) population: hr=0.72 (0.59-0.87)
     - note that n=15 subjects had missing CPS
  - Subjects with CPS>1 and tumor_size>19 (n=363): 
      - *adjusted* hr=0.63 (0.47-0.84)
  - Subjects with CPS<=1 or tumor_size<=19 (n=170): 
      - *adjusted* hr=0.94 (0.58-1.52)
...
```{r SG_outcomes, fig.width=9, fig.height=9}
load(file="d0_analysis/df0_ITT.Rdata")

# Restrict to SCC
df.analysis <- subset(df_itt,scc==1)

dfa <- subset(df.analysis,!is.na(cps))
...
title(sub="(a) OS in overall ITT population")

kmH.fit<-KM.plot.2sample.weighted(df=subset(dfa,cps>1 & tmrsize>19), 
tte.name=outcome.name, event.name=event.name, treat.name=treat.name,
risk.set=TRUE,by.risk=byrisk,risk.cex=0.80,
risk_offset=0.15, risk_delta=0.075,
Xlab="Months",Ylab="Overall survival",details=FALSE,
...
show.med=TRUE, med.cex=0.80, ymed.offset=0.30,
show.cox=TRUE, cox.digits=2, cox.eps=0.0001, show_arm_legend=TRUE,arms=c("Pembro+CT","CT"),arm.cex=0.70)
title(sub="(b) OS in CPS > 1 & tumor size > 19 population")
...
show.med=TRUE, med.cex=0.80, ymed.offset=0.30,
show.cox=TRUE, cox.digits=2, cox.eps=0.0001, show_arm_legend=TRUE,arms=c("Pembro+CT","CT"),arm.cex=0.70)
title(sub="(c) OS in CPS <= 1 or tumor size <= 19 population")

```


### Forestplot by CPS levels 

Hazard ratio estimates by subgroups based on CPS levels (as conducted in recent FDA summary):

```{r fplot1, fig.width=9, fig.height=6}
# simfunctions for (tailor made) forestplot
source("Extreme-SGs/sim_functions_v0.R")
library(grid)
library(forestploter)

# forest plot by CPS
temp <- SG_forest(df=df.analysis,outcome.name="os_time",event.name="os_event",treat.name="combo",xlim=c(0.5,1.5))
...
df.analysis$metastatic <- ifelse(df.analysis$dxstat=="Metastatic",1,0)

df.analysis <- subset(df.analysis,!is.na(cps))

confounders.name <- c("cps","age","tmrsize","male","adeno","ecog0","livermets","metastatic","asia","cps_1","cps_5","cps_10")
...
# SG1
load("outcome/results/sg1_v0b.Rdata")
load("outcome/results/sg1_boots=1000_v0b.Rdata")
load("outcome/results/sg1_OOB_v0b.Rdata")
df_sg <- fs.est$df.est
fs_OOB <- fs_OOB$resCV
df_sg$treat <- df_sg[,c(treat.name)]
# Benefitting (B)
# note: pad name to give more room for longer names
sg1_name <- c("                   CPS > 3 or tumor size > 62")
# Questionable (Q)
sg0_name <- c("CPS <= 3 & tumor size <= 62")
...
load("outcome/results/sg1_20fold_v0b.Rdata")
...
##################################
load("outcome/results/sg3_v0b.Rdata")
load("outcome/results/sg3_boots=1000_v0b.Rdata")
load("outcome/results/sg3_OOB_v0b.Rdata")
df_sg <- fs.est$df.est
fs_OOB <- fs_OOB$resCV
...
load("outcome/results/sg3_20fold_v0b.Rdata")
...
# Reverse the role of treatment
##################################
load("outcome/results/sg4_v0b.Rdata")
load("outcome/results/sg4_boots=1000_v0b.Rdata")
load("outcome/results/sg4_OOB_v0b.Rdata")
df_sg <- fs.est$df.est
fs_OOB <- fs_OOB$resCV
df_sg$treat <- 1-df_sg[,c(fs.est$treat.name)]
sg1_name <- c("CPS > 1 & tumor size > 19")
sg0_name <- c("CPS <= 1 or tumor size <= 19")
...
load("outcome/results/sg4_20fold_v0b.Rdata")
...
res_cps3 <- SG_HRtable2(dfa=subset(dfa,cps>3),df.OOB=NULL,fa_SG=NULL,ntreat_fa=NA,ncontrol_fa=NA,
                         outcome.name=outcome.name,event.name=event.name,treat.name=treat.name,
                         sg_name="CPS>3",E.name=E.name,C.name=C.name)


res_ts19 <- SG_HRtable2(dfa=subset(dfa,tmrsize>19),df.OOB=NULL,fa_SG=NULL,ntreat_fa=NA,ncontrol_fa=NA,
                         outcome.name=outcome.name,event.name=event.name,treat.name=treat.name,
                         sg_name="Tmrsize>19",E.name=E.name,C.name=C.name)


res_ts62 <- SG_HRtable2(dfa=subset(dfa,tmrsize>62),df.OOB=NULL,fa_SG=NULL,ntreat_fa=NA,ncontrol_fa=NA,
                         outcome.name=outcome.name,event.name=event.name,treat.name=treat.name,
                         sg_name="Tmrsize>62",E.name=E.name,C.name=C.name)



# Concatentate the subgroups

dt <- rbind(res_itt,res_cps1,res_cps3,res_ts19,res_ts62,zz,res_sg1,res_sg2,res_sg3)
kfix <- 5
kposthoc <- 3
...
load("outcome/results/sg4_CV_example.Rdata")
Kfolds <-20
...
We summarize the results for a *single* cross-validation iteration for $\hat{B}$ corresponding to **subjects with CPS>1 & tmrsize>19**:  Among the $20$ cross-validation analyses `r SGfound` identified a subgroup (`r 100*round(SGfound/20,2)`\%); of which 5/20 (`r 100*round(5/20,2)`\%) were defined exactly as the full analysis (**CPS>1 & tmrsize>19**), 1/20 were defined as 
**CPS>1 and tmrsize>28.2**, 1/20 defined as **CPS>2.5 and tmrsize>19**, 5/20 defined as **Asia and Male**, 2/20 defined as **CPS>=10**, 1/20 defined as **CPS>=1 & Asia**, 1/20 defined as **CPS>3 & Metastatic**, 1/20 defined as **CPS>2 and Male**, 1/20 as **Tmrsize>19 and Asia**, and 2/20 defined as **Tmrsize>19 & Age<=71**.  There were 
...
Since cross-validation results will generally depend on the random partition, we repeat the cross-validation analysis across 5 repetitions and report the medians of the summaries.  


### KM Difference plots: ITT and subgroups 

The solid black line denotes the ITT Kaplan-Meier treatment difference estimates along with $95\%$ CIs (the grey shaded region).  K-M differences 
corresponding to subgroups are displayed.  Note that these estimates are not adjusted for SGs.


```{r OSdiffs, fig.width =8, fig.height=6, fig.align="center"}


df_itt$cpstsize <- with(df_itt,ifelse(cps>1 & tmrsize>19,1,0))
...
cpslt1_col <- c("red")
cps19_col <- c("orange")
cpsnew_col <- c("lawngreen")
sg_cols <- c(cps10_col,cps19_col,cps5_col,cps1_col,cpslt1_col,cpsnew_col)

temp <- plotKM.band_subgroups(df=subset(df_itt,scc==1),sg_labels=c("cps_10","cps_19","cps_5","cps_1","cps_lt1","cpstsize"),
sg_colors=c(sg_cols),
color=c("azure3"),xlabel=c("Months"),ylabel=c("Survival differences"),
yseq_length=5,
tau_add=30, by.risk=6, risk_cex=0.75, risk_delta=0.0225,
outcome.name=outcome.name, treat.name=treat.name, event.name=event.name)

legend("topleft",c("ITT","cps>=10","cps=1-9","cps>=5","cps>=1","cps<1","cps>1 & tsize>19"),
