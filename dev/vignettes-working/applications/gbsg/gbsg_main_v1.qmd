...
system.time({fs <- forestsearch(df.analysis,  confounders.name = confounders.name,
                                ...
                                sg_focus = "maxSG", max_subgroups_search = 50,
...
fs_OOB <- forestsearch_Kfold(fs.est = fs, details = TRUE, parallel_args = list(plan = "callr", workers = 36, 
        show_message = TRUE))
...
# Append sens_summary to fs_OOB
fs_OOB$sens_summary <- summary_OOB$sens_metrics_original
...
fs_ten <- forestsearch_tenfold(fs.est = fs, sims = 5, Kfolds = 10, details=TRUE, parallel_args = list(plan = "callr", workers = 36, 
        show_message = TRUE))

fs_ten_legacy <- forestsearch_tenfold_legacy(fs.est = fs, sims = 5, Kfolds = 10, details=TRUE)
...
# Create the forest plot
 result <- plot_subgroup_results_forestplot(
   fs_results = list(fs.est = fs, fs_bc = fs_bc, fs_OOB = fs_OOB, cv_source="oob"),
...
# Create the forest plot
 result <- plot_subgroup_results_forestplot(
   fs_results = list(fs.est = fs, fs_bc = fs_bc, fs_kfold = NULL),
   df_analysis = df.analysis,
   subgroup_list = subgroups,
   outcome.name = "time_months",
   event.name = "status",
