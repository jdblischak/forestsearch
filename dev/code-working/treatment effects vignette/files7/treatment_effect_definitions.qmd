...
comparison_tbl <- gt(comparison_df) |>
  cols_label(
    Property = "",
...
# Render as self-contained HTML for pkgdown compatibility
htmltools::browsable(
  htmltools::HTML(
    as.character(gt::as_raw_html(comparison_tbl, inline_css = TRUE))
  )
)
```
...
htmltools::browsable(
  htmltools::HTML(
    as.character(gt::as_raw_html(pipeline_tbl, inline_css = TRUE))
  )
)
```
