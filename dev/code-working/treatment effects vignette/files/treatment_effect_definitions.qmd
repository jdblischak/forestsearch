...
$$
h(t \mid \mathbf{X}_i) = h_0(t) \exp\!\bigl(\mathbf{X}_i^{\top}
...
= \exp\!\Bigl(\hat{\beta}_{\text{Cox}} \;\Big|\;
\text{data} = \bigl\{\bigl(T_i(1), 1, 1\bigr), \bigl(T_i(0), 1, 0\bigr)
: i \in \mathcal{S}\bigr\}\Bigr)
$$
...
\bigl[\theta_i(1) - \theta_i(0)\bigr]\right)
= \exp\!\left(\frac{1}{|\mathcal{S}|} \sum_{i \in \mathcal{S}}
...
\sum_{i \in \mathcal{S}} \exp\!\bigl(\theta_i(1)\bigr)}
{\frac{1}{|\mathcal{S}|}
\sum_{i \in \mathcal{S}} \exp\!\bigl(\theta_i(0)\bigr)}
...
$$
\text{HR}(\mathcal{H}) = \exp\!\bigl(\beta_0[\text{treat}] +
\beta_0[\text{zh}]\bigr)
$$

$$
\text{HR}(\mathcal{H}^c) = \exp\!\bigl(\beta_0[\text{treat}]\bigr)
$$
...
#| tbl-cap: "Comparison of Treatment Effect Definitions"
#| echo: true
...
```{r}
#| label: tbl-pipeline
#| tbl-cap: "Treatment Effect Computation Pipeline"
#| echo: true
