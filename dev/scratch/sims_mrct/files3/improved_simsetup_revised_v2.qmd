...
- "AP" (Asia Pacific) renamed to "regA" (Region A) for generalizability
- "z_AP" renamed to "z_regA" for consistency with naming convention
- `confounder_var`/`confounder_effect` parameters allow specifying **any** prognostic confounder effect (separate from region used for train/test splitting)
- **No regional prognostic effect by default** - region is only used for splitting data
- Function `mrct_APregion_sims` renamed to `mrct_region_sims`
...
#' @param region_var Region indicator variable name used for splitting data into
...
mrct_region_sims <- function(
  ...
  results <- with_progress({
    ...
    foreach(
      ...
      hr_ittX <- safe_coxph(
        as.formula(paste0("Surv(y_sim, event_sim) ~ treat_sim + strata(", region_var, ")")), 
        dfs
      )
...
summaryout_mrct <- function(
    ...
    out_est = "+ regAflag + sg_le85 + regAflag2 + regAflag3 + hr_sg_null",
    sg_type = 1,
...
```{r data-prep, echo=TRUE, message=FALSE, warning=TRUE, eval=FALSE}
...
## Example 1: Simulations with NO Forced Confounder Effect (Default)

By default, `create_dgm_for_simulations()` does NOT include any forced prognostic confounder effect. The region variable (`regA`) is included in the model but its effect is estimated from the data rather than forced to a specific value.
...
# Create DGM with strong biomarker effect (NO forced confounder effect)
...
mrct_sims_alt <- mrct_region_sims(
  dgm = dgm_alt,
  n_sims = n_sims,
  region_var = "z_regA",
  sg_focus = "minSG",
...
## Example 2: Simulations with a Forced Confounder Effect

If you want to include a forced prognostic effect for a specific confounder (e.g., `prior_treat`), use the `confounder_var` and `confounder_effect` parameters:

```{r sims_confounder, echo=TRUE, message=FALSE, warning=TRUE, eval=FALSE}
#| label: sims-with-confounder

# Create DGM with a forced confounder effect on prior_treat
...
# Run simulations with the confounder effect
mrct_sims_conf <- mrct_region_sims(
  dgm = dgm_with_confounder,
  n_sims = n_sims,
  region_var = "z_regA",
  sg_focus = "minSG",
  maxk = 1,
...
## Example 3: Simulations under Null (Uniform Effect)
...
# Create DGM with uniform effect (null hypothesis, no forced confounder)
...
mrct_sims_null <- mrct_region_sims(
  dgm = dgm_null,
  n_sims = n_sims,
  region_var = "z_regA",
  sg_focus = "minSG",
...
|----------|---------|-------------|
| `AP` | `regA` | Region indicator variable in data |
| `z_AP` | `z_regA` | Transformed region variable |
| `df_nonAP` | `df_nonRegA` | Non-region A (training) data frame |
| `df_AP` | `df_regA` | Region A (testing) data frame |
| `mrct_APregion_sims` | `mrct_region_sims` | Main simulation function |

## Parameter Changes

| Original | Revised | Description |
|----------|---------|-------------|
| `region_effect` | `confounder_effect` | Effect size for optional confounder |
| `region_var` (in DGM) | `confounder_var` | Name of confounder variable |
| `wname`, `bw` | Removed | No longer needed - use `confounder_var`/`confounder_effect` |

## Key Design Principle: Separation of Concerns

The revised code separates two distinct concepts:

1. **Region for Train/Test Split** (`region_var` in `mrct_region_sims()`)
   - Used ONLY to split data into training (non-Region A) and testing (Region A)
   - Does NOT imply any prognostic effect in the outcome model

2. **Optional Confounder Effect** (`confounder_var`/`confounder_effect` in `create_dgm_for_simulations()`)
   - Allows forcing a prognostic effect for ANY baseline covariate
   - Can be any variable (not necessarily region)
   - Default is NULL (no forced effect)

This separation allows maximum flexibility:
- By default: No forced confounder effects, region only used for splitting
- Optionally: Force effect on any confounder variable of interest
...
| Variable naming | Mixed conventions (AP) | Consistent `z_` prefix (regA) |
...
| Region naming | AP-specific | Generic (regA) |
| Confounder effect | Forced regional effect | Optional, any variable |
