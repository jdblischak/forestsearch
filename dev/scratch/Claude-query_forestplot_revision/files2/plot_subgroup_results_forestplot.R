...
#'   \itemize{
#'     \item \code{subset_expr}: Character string for subsetting (e.g., "cps > 1")
...
#'     type = "reference"
#'   ),
#'   cps_gt1_size_gt19 = list(
#'     subset_expr = "cps > 1 & tmrsize > 19",
#'     name = "CPS > 1 & Tumor Size > 19",
#'     type = "posthoc"
#'   )
#' )
#' 
...
plot_subgroup_results_forestplot <- function(
  ...
  if (!is.null(subgroup_list)) {
    ...
    # FIX: Process posthoc entries (was missing in original)
    # -----------------------------------------------------------------
    ...
    if (length(posthoc_sgs) > 0) {
      ...
      # FIX: Loop through and add each posthoc subgroup from subgroup_list
  ...
  # FIX: Correct labeling (sg.harm defines H, not benefit!)
  # -----------------------------------------------------------------
  if (!is.null(fs.est) && !is.null(fs.est$df.est)) {
    ...
    if (est.scale == "hr") {
      ...
      # FIX: benefit is COMPLEMENT of sg.harm, question IS sg.harm
    ...
    # CV metrics for benefit subgroup
