...
#'     \item \code{fs_OOB}: Out-of-bag cross-validation results (optional)
...
#' @param title_text Character. Plot title (default: "Subgroup Treatment Effects").
#' @param arrow_text Character vector of length 2. Arrow labels for forest plot
#'   (default: c("Favors Experimental", "Favors Control")).
#' @param footnote_text Character. Footnote text for the plot (optional).
...
#'   (default: TRUE if fs_kfold available).
...
plot_subgroup_results_forestplot <- function(
    ...
    C.name = "Control",
    est.scale = "hr",
    title_text = "Subgroup Treatment Effects",
  ...
  if (is.null(fs.est)) {
    stop("fs_results must contain 'fs.est' from forestsearch()")
  }
  ...
  create_hr_row <- function(dfa, sg_name, outcome.name, event.name, treat.name,
    ...
    fit <- survival::coxph(cox.formula, data = dfa)
  ...
  create_bc_row <- function(bc_estimates, sg_name, ntreat, ncontrol, E.name, C.name) {
    ...
    row <- data.frame(
      Subgroup = paste0("  ", sg_name, " (bias-corrected)"),
      stringsAsFactors = FALSE
    )
    row[[E.name]] <- ntreat
    row[[C.name]] <- ncontrol
  ...
  generate_sens_text <- function(fs_kfold, est.scale) {
  ...
  if (!is.null(subgroup_list)) {
    ...
    # Add separator before post-hoc subgroups
  ...
  # Add identified subgroups from ForestSearch
  if (!is.null(fs.est) && !is.null(fs.est$df.est)) {
    ...
    if (!is.null(sg_harm)) {
      sg1_label <- paste(sg_harm, collapse = " & ")
    } else {
      sg1_label <- "Identified Subgroup"
    }

    # Add benefitting subgroup (Hc for hr scale, H for 1/hr scale)
    if (est.scale == "hr") {
      df_benefit <- subset(df_sg, treat.recommend == 1)
      df_question <- subset(df_sg, treat.recommend == 0)
      benefit_name <- sg1_label
      question_name <- paste0("NOT (", sg1_label, ")")
    } else {
      df_benefit <- subset(df_sg, treat.recommend == 0)
      df_question <- subset(df_sg, treat.recommend == 1)
      benefit_name <- sg1_label
      question_name <- paste0("NOT (", sg1_label, ")")
    }

    # Benefitting subgroup header
    benefit_header <- create_header_row(
      paste0("                   ", benefit_name), E.name, C.name)
    dt <- rbind(dt, benefit_header)
    row_types <- c(row_types, "posthoc_header")

    # Benefitting subgroup - unadjusted
    if (nrow(df_benefit) > 10) {
      ...
      if (!is.null(fs_bc)) {
        bc_est <- if (est.scale == "hr") fs_bc$Hc_estimates else fs_bc$H_estimates
        if (!is.null(bc_est)) {
    ...
    # CV metrics for benefitting subgroup
    if (!is.null(fs_kfold) && show_cv_metrics) {
      cv_text <- generate_sens_text(fs_kfold, est.scale)
      if (!is.null(cv_text)) {
        cv_texts[[length(cv_texts) + 1]] <- cv_text
        cv_row_positions[[length(cv_row_positions) + 1]] <- nrow(dt) + 1
      }
    }

    # Questionable subgroup header
    question_header <- create_header_row(question_name, E.name, C.name)
    dt <- rbind(dt, question_header)
    row_types <- c(row_types, "posthoc_header")

    # Questionable subgroup - unadjusted
    if (nrow(df_question) > 10) {
      ...
      # Add bias-corrected if available
      if (!is.null(fs_bc)) {
        bc_est <- if (est.scale == "hr") fs_bc$H_estimates else fs_bc$Hc_estimates
        if (!is.null(bc_est)) {
  ...
  # Add CV metrics text annotations if available
  g <- p
