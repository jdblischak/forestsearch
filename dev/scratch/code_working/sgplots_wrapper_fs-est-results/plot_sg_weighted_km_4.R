...
plot_sg_weighted_km <- function(
 ...
 if (!is.null(sg_definition_label)) {
  ...
  # Add bias-corrected HR explanation if bc estimates were used
  if (!is.null(H_bc_estimates) || !is.null(Hc_bc_estimates)) {
...
figure_note <- function(x,
  ...
  # Add HR(bc) explanation
  if (include_hr_explanation && (!is.null(x$H_bc_estimates) || !is.null(x$Hc_bc_estimates))) {
    note_parts <- c(note_parts, "HR(bc) = bootstrap bias-corrected hazard ratio.")
  }
