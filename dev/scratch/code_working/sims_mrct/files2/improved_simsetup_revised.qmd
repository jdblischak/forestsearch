...
- `wname` and `bw` parameters kept as general prognostic confounder controls (separate from region identification)
...
#' @param region_var Region indicator variable name (default: "z_regA")
...
#' @param bw Log HR for prognostic effect of wname variable
...
create_dgm_for_simulations <- function(
  ...
  # Build z_ prefixed region variable name
  ...
  generate_aft_dgm_flex(
    ...
    set_beta_spec = list(set_var = z_region_var, beta_var = region_effect),
...
| `ap_effect` | `region_effect` | DGM parameter for region prognostic effect |
| `APflag` | `regAflag` | Summary flag variables |

## Parameter Clarification

The `wname` and `bw` parameters have been clarified as **general prognostic confounder controls** that are separate from region identification:

- **`region_var`**: Controls which variable is used to split data into training (non-Region A) and testing (Region A) populations for subgroup identification
- **`wname`**: (Optional) Name of a prognostic confounder variable in the outcome model - can be any baseline covariate
- **`bw`**: Log hazard ratio for the prognostic effect of the `wname` variable (if specified in DGM)

This separation allows flexibility where:
1. The region split for subgroup analysis can be based on one variable
2. The prognostic effect in the outcome model can be based on a different (or the same) variable
