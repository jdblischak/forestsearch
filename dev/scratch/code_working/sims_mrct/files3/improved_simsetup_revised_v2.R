...
#' - confounder_var/confounder_effect parameters allow specifying any prognostic
#'   confounder effect (separate from region used for train/test splitting)
#' - No regional prognostic effect by default
#' =============================================================================
...
#' @param region_var Name of the region indicator variable used to split data into 
#'        training (non-Region A) and testing (Region A) populations (default: "z_regA")
...
#' 
#' Note: The region_var parameter is used ONLY for splitting data into training/testing.
#' Any prognostic confounder effects should be specified when creating the DGM via
#' create_dgm_for_simulations() using confounder_var and confounder_effect parameters.
#' -----------------------------------------------------------------------------
...
#' @param confounder_var Name of a confounder variable to include with a forced 
#'        prognostic effect (optional; default NULL = no forced confounder effect)
#' @param confounder_effect Log hazard ratio for the confounder_var effect
#'        (only used if confounder_var is specified)
#' @param include_regA Logical: include regA as a factor in the model (default TRUE)
#' @param verbose Print detailed output
#' 
#' @return DGM object for use with simulate_from_dgm()
#' 
#' @details
#' By default, NO prognostic confounder effect is forced. The confounder_var and
#' confounder_effect parameters allow optionally specifying ANY baseline covariate
#' to have a forced prognostic effect in the outcome model.
#' 
#' The regA variable (region indicator) is included as a factor in the model by
#' default (include_regA=TRUE) but without a forced effect - its effect will be
#' estimated from the data. Set include_regA=FALSE to exclude it entirely.
#' 
#' To add a prognostic confounder effect (e.g., for a variable like "smoking"):
#'   create_dgm_for_simulations(df_case, confounder_var = "smoking", 
...
# 
# # -----------------------------------------------------------------------------
# # Example 1: DGM with NO forced confounder effect (default)
# # -----------------------------------------------------------------------------
...
# 
# # -----------------------------------------------------------------------------
# # Example 2: DGM with a forced confounder effect on a specific variable
# # (e.g., forcing a prognostic effect for "prior_treat")
# # -----------------------------------------------------------------------------
# dgm_with_confounder <- create_dgm_for_simulations(
#   df_case = df_case,
#   model_type = "alt",
#   log_hrs = log(c(3, 1.25, 0.50)),
#   confounder_var = "prior_treat",
#   confounder_effect = log(1.5),  # HR = 1.5 for prior_treat
#   verbose = TRUE
# )
# 
# # -----------------------------------------------------------------------------
# # Example 3: DGM for null hypothesis (uniform effect, no confounder effect)
# # -----------------------------------------------------------------------------
