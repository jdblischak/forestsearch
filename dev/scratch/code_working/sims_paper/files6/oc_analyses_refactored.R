...
utils::globalVariables(c(
  ...
  # Data frame columns created during analysis
  "sg_hat"
))
...
run_forestsearch_analysis <- function(
  ...
  if (has_result) {
    extract_fs_estimates(
      ...
      fs_full = fs_result,  # Pass full result for df.est access
      dgm = dgm,
      cox_formula = cox_formula,
      cox_formula_adj = cox_formula_adj,
...
run_grf_analysis <- function(
  ...
  # Run GRF analysis using the retrieved function
  grf_result <- tryCatch({
    grf_fun(
      data = data,
      confounders.name = confounders_name,
...
# =============================================================================
# gt Table Generation for Simulation Results
# =============================================================================

#' Format Operating Characteristics Results as gt Table
#'
#' Creates a formatted gt table from simulation results summary. This function
#' is specifically for displaying operating characteristics from simulation
#' studies (e.g., subgroup detection rates, classification metrics).
#'
...
#'
#' @importFrom gt gt tab_header tab_style cell_fill cells_row_groups 
#'   tab_options px tab_source_note cols_label
#' @export
...
format_oc_results <- function(
 ...
 if (!requireNamespace("gt", quietly = TRUE)) {
    warning("Package 'gt' is required for formatted tables. Returning data.frame.")
    return(summary_df[row_subset, , drop = FALSE])
  }
 ...
 # Add row names as column for gt
  summary_df$Metric <- rownames(summary_df)
  rownames(summary_df) <- NULL
  
  # Reorder columns to put Metric first
  col_order <- c("Metric", setdiff(names(summary_df), "Metric"))
  summary_df <- summary_df[, col_order]
  
  # Add grouping variable
  summary_df$group <- c(
    rep("Classification Metrics", 5),
    rep("Subgroup Sizes", 6)
  )
  
  # Create gt table
  tbl <- gt::gt(summary_df, groupname_col = "group") |>
    gt::tab_header(
  ...
  if (!is.null(power_approx)) {
    tbl <- gt::tab_source_note(
      tbl,
      source_note = sprintf("Theoretical power approximation: %.3f", power_approx)
    )
  }
  
  tbl <- gt::tab_source_note(
    tbl,
    source_note = sprintf("Number of simulations: %d", n_sims)
  )
  
  # Add note for null model
  if (model_type == "null") {
    tbl <- gt::tab_source_note(
      tbl,
      source_note = "Note: Under the null hypothesis, sens(H) is undefined."
    )
  }
