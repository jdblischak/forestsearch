...
1. **Download all files** from this directory to your package root
2. **Run the automated fix script:**
   ```r
   # From your package root directory (where DESCRIPTION is)
...
### Option 2: Manual Fix for One File

1. **Download** `bootstrap_summaries_helpers.R`
2. **Replace** your original file:
   ```bash
   cp bootstrap_summaries_helpers.R ~/Documents/GitHub/forestsearch/R/
   ```
3. **Repeat** for other files using the same approach
...
### Tools
- **fix_all_nonascii.R** - Automated R script to fix all 9 files at once
