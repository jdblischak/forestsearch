...
subgroup.consistency <- function(df,
 ...
 if (!is.logical(use_twostage) || length(use_twostage) != 1) {
  ...
  # Create evaluation function based on algorithm choice
  if (use_twostage) {
    # Two-stage sequential evaluation function
    eval_fun <- function(m) {
    ...
    # Functions needed for parallel execution
    ...
    # Original fixed-sample evaluation function
    eval_fun <- function(m) {
  ...
  if (!use_parallel) {
    ...
    results_list <- lapply(seq_len(n_candidates), eval_fun)
    ...
    results_list <- future.apply::future_lapply(
      ...
      future.globals = structure(
        ...
        add = c(
          ...
          "maxk",
          "confs_labels",
          # Two-stage specific (included even if not used - won't cause issues)
          "ts_params"
        )
      )
    )
