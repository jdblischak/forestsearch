# =============================================================================
# Operating Characteristics Analysis Functions for GBSG Simulations
# =============================================================================
#
...
#   2. Source this file: source("oc_analyses_gbsg.R")
#   3. Run simulations
#
# =============================================================================
...
run_forestsearch_analysis <- function(
  ...
  if (verbose) {
    ...
    message(sprintf("  [%s] Parameters: use_lasso = %s, use_grf = %s, hr.threshold = %.2f",
                    analysis_label, params$use_lasso, params$use_grf, params$hr.threshold))
  }
 ...
 # Build forestsearch call arguments
  ...
  # Valid forestsearch parameters only
  param_names <- c(
    ...
    "frac.tau", "dmin.grf", "grf_depth", "use_twostage", "twostage_args"
  )
...
run_simulation_analysis <- function(
 ...
 # -------------------------------------------------------------------------
  ...
  if (show_verbose) {
    ...
    message(sprintf("    DGM HR_H = %.3f, HR_Hc = %.3f", dgm$hr_H_true, dgm$hr_Hc_true))
  }
...
#' hazard ratio exceeding threshold based on asymptotic normal approximation.
#'
