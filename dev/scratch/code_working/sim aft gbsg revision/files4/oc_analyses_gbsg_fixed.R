...
#' - sg.harm.id or sg_harm_id: subgroup definition string
...
extract_grf_estimates <- function(
  ...
  # FORCED DIAGNOSTIC - always print to diagnose the issue
  message(sprintf("  [%s] extract_grf_estimates called. verbose=%s, debug=%s", 
                  analysis, verbose, debug))
  message(sprintf("  [%s] grf_est is NULL: %s", analysis, is.null(grf_est)))
  if (!is.null(grf_est)) {
    message(sprintf("  [%s] grf_est names: %s", analysis, paste(names(grf_est), collapse = ", ")))
    message(sprintf("  [%s] 'sg.harm.id' in names: %s", analysis, "sg.harm.id" %in% names(grf_est)))
    if ("sg.harm.id" %in% names(grf_est)) {
      message(sprintf("  [%s] sg.harm.id value: '%s' (class: %s, length: %d)", 
                      analysis, 
                      paste(grf_est$sg.harm.id, collapse = ", "),
                      class(grf_est$sg.harm.id),
                      length(grf_est$sg.harm.id)))
    }
    message(sprintf("  [%s] 'data' in names: %s", analysis, "data" %in% names(grf_est)))
    if ("data" %in% names(grf_est)) {
      message(sprintf("  [%s] 'treat.recommend' in data: %s", 
                      analysis, "treat.recommend" %in% names(grf_est$data)))
    }
  }
  ...
  # If no GRF result
  if (is.null(grf_est)) {
    message(sprintf("  [%s] EARLY RETURN: grf_est is NULL", analysis))
    out$hr.Hc.hat <- out$hr.itt
    out$ahr.Hc.hat <- compute_ahr(df)
    return(out)
  }
  ...
  if ("sg.harm.id" %in% names(grf_est)) {
    ...
    message(sprintf("  [%s] Found sg.harm.id: '%s'", analysis, paste(sg_harm_id, collapse = ", ")))
  }
  # Try sg_harm_id (with underscores)
  if (is.null(sg_harm_id) && "sg_harm_id" %in% names(grf_est)) {
    sg_harm_id <- grf_est$sg_harm_id
    message(sprintf("  [%s] Found sg_harm_id: '%s'", analysis, paste(sg_harm_id, collapse = ", ")))
  }
  ...
  message(sprintf("  [%s] has_sg_harm_id: %s", analysis, has_sg_harm_id))
  ...
  message(sprintf("  [%s] has_treat_recommend: %s", analysis, has_treat_recommend))
  
  has_subgroup <- has_sg_harm_id && has_treat_recommend
  
  message(sprintf("  [%s] has_subgroup (final): %s", analysis, has_subgroup))
  
  if (!has_subgroup) {
    message(sprintf("  [%s] EARLY RETURN: No subgroup detected", analysis))
    out$hr.Hc.hat <- out$hr.itt
    out$ahr.Hc.hat <- compute_ahr(df)
    return(out)
  }
  ...
  # -------------------------------------------------------------------------
  out$any.H <- 1L
  message(sprintf("  [%s] SUCCESS: Setting any.H = 1", analysis))
  ...
  message(sprintf("  [%s] Subgroup: %s", analysis, paste(sg_harm_id, collapse = " & ")))
  message(sprintf("  [%s] size.H = %d, size.Hc = %d", analysis, out$size.H, out$size.Hc))
  ...
  if (is.null(outcome_col) || is.null(event_col) || is.null(treat_col)) {
    message(sprintf("  [%s] Warning: Missing columns in grf_data, falling back to df", analysis))
    outcome_col <- "y.sim"
    event_col <- "event.sim"
    treat_col <- "treat"
    grf_data <- df
  ...
  if (out$size.H > 10 && sum(grf_data$sg_hat == 1 & grf_data[[event_col]] == 1) >= 5) {
    out$hr.H.hat <- tryCatch({
      ...
      message(sprintf("  [%s] HR(H) estimation failed: %s", analysis, e$message))
      NA_real_
    })
  }
  ...
  if (out$size.Hc > 10 && sum(grf_data$sg_hat == 0 & grf_data[[event_col]] == 1) >= 5) {
    out$hr.Hc.hat <- tryCatch({
      ...
      message(sprintf("  [%s] HR(Hc) estimation failed: %s", analysis, e$message))
      NA_real_
    })
  }
  ...
  message(sprintf("  [%s] FINAL any.H = %d", analysis, out$any.H))
