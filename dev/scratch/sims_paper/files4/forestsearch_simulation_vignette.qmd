...
sim_data <- simulate_from_gbsg_dgm(
  ...
  sim_id = 1,
  max_follow = 60,       # 60 months administrative censoring
  muC_adj = 0            # No censoring adjustment
)
...
```{r examine-covariates}
# Summary of key variables
summary(sim_data[, c("y.sim", "event.sim", "treat", "flag.harm", 
                     "z1", "z3")])
```

```{r plot-km, fig.height=5, fig.width=8}
# Kaplan-Meier by treatment
library(survminer)

fit_km <- survfit(Surv(y.sim, event.sim) ~ treat, data = sim_data)
...
ggsurvplot(
  ...
  xlab = "Time (months)",
  title = "Overall Treatment Effect (Simulated Data)"
)
```
...
# Simulation settings
sim_config <- list(
  n_sims = 200,          # Number of simulations (use 500-1000 for final)
  n_sample = 500,        # Sample size per trial
  max_follow = 60,       # Maximum follow-up (months)
  seed_base = 8316951
)
...
results_alt <- foreach(
  ...
  run_simulation_analysis(
    ...
    run_fs = TRUE,
    run_fs_grf = TRUE,
    run_grf = FALSE,      # Set TRUE if grf_subg_harm_survival available
    fs_params = fs_params,
...
The function `compute_theoretical_power()` provides an analytical approximation
...
prop_cens <- 1 - mean(results_alt$p.cens)

# Theoretical power
power_approx <- compute_theoretical_power(
...
cat("\n=== Power Comparison ===\n")
cat("Theoretical approximation:", round(power_approx, 3), "\n")
...
```{r power-curve, fig.height=5, fig.width=8}
# Calculate power for different sample sizes
sample_sizes <- seq(200, 1000, by = 100)
power_curve <- data.frame(
  n = sample_sizes,
  power = sapply(sample_sizes, function(n) {
    n_sg <- n * mean(dgm_calibrated$df_super_rand$flag.harm)
...
ggplot(power_curve, aes(x = n, y = power)) +
  ...
  geom_vline(xintercept = sim_config$n_sample, linetype = "dotted", color = "gray50") +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  labs(
    x = "Sample Size",
    y = "Approximate Power",
    title = "Power Curve for Subgroup Detection",
    subtitle = sprintf("HR(H) = %.2f, threshold = %.2f",
                       dgm_calibrated$hr_H_true, fs_params$hr.threshold)
  ) +
  theme_minimal()
```
...
```{r power-hr-curve, fig.height=5, fig.width=8}
# Calculate power for different HRs
hr_values <- seq(1.1, 2.5, by = 0.1)
n_sg_fixed <- sim_config$n_sample * mean(dgm_calibrated$df_super_rand$flag.harm)

power_hr <- data.frame(
  hr = hr_values,
  power = sapply(hr_values, function(hr) {
    compute_theoretical_power(
      n_sg = round(n_sg_fixed),
      prop_cens = 0.35,
      theta = hr,
      hr_threshold = fs_params$hr.threshold
    )
...
ggplot(power_hr, aes(x = hr, y = power)) +
  geom_line(linewidth = 1.2, color = "darkgreen") +
  geom_point(size = 2, color = "darkgreen") +
  geom_hline(yintercept = 0.80, linetype = "dashed", color = "red") +
  geom_vline(xintercept = dgm_calibrated$hr_H_true, linetype = "dotted", color = "gray50") +
  geom_vline(xintercept = fs_params$hr.threshold, linetype = "dashed", 
             color = "orange", alpha = 0.7) +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  labs(
    x = "True Hazard Ratio in Harm Subgroup",
    y = "Approximate Power",
    title = "Power vs Effect Size",
    subtitle = sprintf("n = %d, threshold = %.2f (orange dashed)",
                       sim_config$n_sample, fs_params$hr.threshold)
  ) +
  theme_minimal()
```
...
results <- foreach(sim = 1:N_SIMS, .combine = rbind,
