...
# Trying to match setup of simulations from paper to achieve 0.701027 under null
# kvals <- seq(0.90, 1.1, by = 0.001)
# hr_val <- rep(NA,length(kvals))
# for(kk in 1:length(kvals)){
# dgm_null_kk <- create_gbsg_dgm(
#   model = "null",
#   k_treat = kvals[kk],
#   verbose = FALSE
# )
# hr_val[kk] <- dgm_null_kk$hr_causal
# }
# plot(kvals,hr_val)
# k_find <- kvals[which(round(hr_val,4) == 0.701)]
# 0.986
...
fs_params <- list(
  ...
  treat.name = "treat",
  id.name = "id",
  parallel_args = list(plan = "multisession", workers = 20, show_message = TRUE),
  use_twostage = FALSE,
  hr.threshold = 1.25,
  hr.consistency = 1.0,
