...
fs_OOB <- forestsearch_Kfold(
  fs.est = fs,
  details = TRUE,
  Kfolds = round(nrow(df.analysis)/150,0),  # N-fold = leave-one-out
  parallel_args = list(
    plan = "multisession",
    workers = n_cores,
    show_message = TRUE
...
footnote_fontsize = 22, cv_fontsize = 22)
...
#render_forestplot(result, width = 6, height = 8)
...
# library(gridExtra)
# grid::grid.newpage()
# grid.arrange(result$plot, newpage = FALSE)
