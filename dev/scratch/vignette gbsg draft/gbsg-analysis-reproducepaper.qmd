...
# The following is automated in wrapper below
# df.analysis$age_lt65 <- with(df.analysis, ifelse(age < 65, 1,0))
# df.analysis$age_ge65 <- with(df.analysis, ifelse(age >= 65, 1,0))
# df.analysis$Qrecommend <- with(df.analysis,ifelse(er <= 0,1,0))
# df.analysis$Brecommend <- with(df.analysis,ifelse(er > 0,1,0))
# age_lt65_col <- c("brown")
# age_ge65_col <- c("grey")
# Brecommend_col <- c("red")
# Qrecommend_col <- c("blue")
# sg_cols <- c(age_lt65_col, age_ge65_col, Brecommend_col,Qrecommend_col)
# temp <- plotKM.band_subgroups(df=df.analysis,sg_labels=c("age < 65", "age >= 65", "er <= 0","er > 0"),
# sg_colors=c(sg_cols),
# color=c("azure3"),xlabel=c("Months"),ylabel=c("Survival differences"),
# yseq_length=5,
# draws.band = 1000,
# tau_add=60, by.risk=6, risk_cex=0.75, risk_delta=0.035, risk.pad=0.015, ymax.pad=0.11, 
# tte.name=outcome.name, treat.name=treat.name, event.name=event.name)
# 
# legend("topleft",c("ITT","age < 65", "age >= 65","er <= 0", "er > 0"),
# lwd=2, col=c("black",sg_cols),bty="n",cex=0.75)


# # Core display of ITT and identified subgroups
...
# # Example with more optional subgroups
# ref_sgs <- list(
