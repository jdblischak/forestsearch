...
Exploratory subgroup analyses based on overall survival outcomes will be conducted.
...
9. `Prior Gastrectomy`
10. `Chemotherapy (Fluorouracil [5-FU], Capecitabine [CAPE])`
...
sg0_name <- c("CPS <= 1 / tumor size < 80")
...
res_lADCape <- SG_HRtable2(dfa=subset(dfa,mAD_fluo5==0),df.OOB=NULL,fa_SG=NULL,ntreat_fa=NA,ncontrol_fa=NA,
                         outcome.name=outcome.name,event.name=event.name,treat.name=treat.name,
                         sg_name="lAD,Cape",E.name=E.name,C.name=C.name)

# Concatentate the subgroups

dt <- rbind(res_itt,res_cps1,res_cps5,res_age65,res_lADCape,zz,res_sg1,res_sg2,res_sg3)
kfix <- 5
kposthoc <- 3
...
sg_cols <- c(cps10_col,cps5_col,cpslt1_col,Brecommend_col,Qrecommend_col)

temp <- plotKM.band_subgroups(df=df.analysis,sg_labels=c("cps_10","cps_5","cps_lt1","Brecommend","Qrecommend"),
sg_colors=c(sg_cols),
color=c("azure3"),xlabel=c("Months"),ylabel=c("Survival differences"),
yseq_length=5,
tau_add=60, by.risk=6, risk_cex=0.75, risk_delta=0.0225, ymax.pad=0.05,
outcome.name=outcome.name, treat.name=treat.name, event.name=event.name)

legend("topright",c("ITT","cps>=10","cps>=5","cps<=1","cps>1 & age<=65","cps<=1 or age>65"),
