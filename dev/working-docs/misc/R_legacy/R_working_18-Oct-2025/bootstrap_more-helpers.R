...
bootstrap_reproduce_aboot <- function(this_boot, fs.est, cox.formula.boot) {
  ...
  if(boot == this_boot){
    ...
    res$events_H_0 <- events_H_0
    res$events_H_1 <- events_H_1
    ...
    res$fitH_star <- fitH_star


    fitH_star2 <- withCallingHandlers(
      {
        get_Cox_sg(
          df_sg = df_H,
          cox.formula = cox.formula.boot,
    ...
    res$events_Hc_0 <- events_Hc_0
    res$events_Hc_1 <- events_Hc_1
...
res$fitHc_star <- fitHc_star
    ...
    res$events_Hstar_0 <- sum(df_Hstar[df_Hstar[[treat_var]] == 0, event_var], na.rm = TRUE)
    res$events_Hstar_1 <- sum(df_Hstar[df_Hstar[[treat_var]] == 1, event_var], na.rm = TRUE)

    # NEW subgroup Hc* on ORIGINAL data
    df_Hcstar <- subset(df_PredBoot, treat.recommend == 1)

    res$events_Hcstar_0 <- sum(df_Hcstar[df_Hcstar[[treat_var]] == 0, event_var], na.rm = TRUE)
    res$events_Hcstar_1 <- sum(df_Hcstar[df_Hcstar[[treat_var]] == 1, event_var], na.rm = TRUE)
