...
mean, SD, range, and relative bias versus the DGM truth.
...
six estimators across two row-groups.
...
undershoots the DGM truth**. Positive values indicate upward bias (common
...
**H header**: `"H: 18 estimable, avg |H| = 90, θ̂(H) = 2.0, âhr(H) = 1.85"`

Built by `build_H_label(n_estimable, avg_size_H, theta_H_true, ahr_H_true)`.

**Hᶜ header**: `"Hᶜ: avg |Hᶜ| = 610, θ̂(Hᶜ) = 0.65, âhr(Hᶜ) = 0.68"`

Built by `build_Hc_label(avg_size_Hc, theta_Hc_true, ahr_Hc_true)`.

The AHR portion is conditionally appended only when `ahr_*_true` is non-NA.
...
- Uniform `font_size` across body cells, row-group headers, and table-level
  font size
...
correction, and AHR columns present:
...
Under the **null hypothesis** the same structure applies, but:
