...
For a subgroup $H$ with underlying log-hazard ratio $\beta$, we can approximate the probability of identifying $H$ via: $P(\text{identify } H) \approx$

$$\int \int I(w_1 + w_2 \geq 2\log(1.25)) \cdot I(w_1 \geq 0) \cdot I(w_2 \geq 0) \cdot \varphi(w_1; \beta, 8/d) \cdot \varphi(w_2; \beta, 8/d) \, dw_1 \, dw_2$$
...
The following is based on monte-carlo simulations.  However, the package also contains a numerical integration implementation that is illustrated in the simulation vignette materials.
```{r}
...
digraph Bootstrap {
  ...
  // Force B and C into the same column
  {rank = same; A; B}
  {rank = same; C; D; E}
