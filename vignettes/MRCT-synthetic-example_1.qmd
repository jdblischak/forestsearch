...
plot_weighted_km(dfc, conf.int = TRUE, show.logrank = TRUE, ymax = 1.05, xmed.fraction = 0.875, ymed.offset = 0.125)
...
dgm_spline <- generate_aft_dgm_flex(
  ...
  # No subgroup-specific effects
  subgroup_vars = NULL,
  subgroup_cuts = NULL,
  k_inter = 0.0,
  
  # Alternative AFT model formulation
  model = "alt",
...
View the censoring parameters:  Using this case example the best fitting censoring model is a common censoring (weibull with NO covariates).   If the selected model included 
...
Note that in `df_super`: 
- `lin_pred_1` = $m(1,z)/\tau$, 
- `lin_pred_0` = $m(0,z)/\tau$, and 
- `loghr_po` = $\psi^{0}(z) = {m(0,z) - m(1,z)}/\tau$ 
...
superpopln_tab <- create_baseline_table(data=dgm_spline$df_super, treat_var="treat", 
...
```{r simulate-from-dgm, eval=FALSE}
...
simdata_tab <- create_baseline_table(data = sim_data, treat_var="treat_sim", 
...
baseline_tab <- create_baseline_table(data=df_large, treat_var="treat_sim", 
                                      vars_continuous=c("z_age","z_bm","entrytime","lin_pred_1","lin_pred_0","loghr_po"),
                                      vars_categorical=c("z_male","z_histology","z_prior_treat","z_AP","flag_harm"))
