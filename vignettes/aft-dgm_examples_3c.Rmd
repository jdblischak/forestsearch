...
# Example: Spline treatment effect on ER with subgroup interaction on meno
dgm_spline <- generate_aft_dgm_flex(
  ...
  event_var = "status",
  treatment_var = "hormon",
  subgroup_vars = "meno",  # Subgroup on meno
  subgroup_cuts = list(meno = 0),
  model = "alt",
  spline_spec = list(
    var = "z_er",  # Use original variable name (or "z_er" if transformed)
    knot = 20,
    zeta = 50,
    log_hrs = log(c(1, 1, 1))  # HR varies with ER
  ),
